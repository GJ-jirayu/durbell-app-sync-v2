...
$paramDisCountFor = "";
//	$paramDiscPer = trim($_POST['paramDiscPer']);
$paramDiscPer = 0;
//	$paramDiscBaht = trim($_POST['paramDiscBaht']);
$paramDiscBaht = 0;
$paramLimitFreeQty = trim($_POST['paramLimitFreeQty']);
//	$paramLimitDiscBath = trim($_POST['paramLimitDiscBath']);
$paramLimitDiscBath = 0;
$paramPriority = $_POST['paramPriority'];
$paramFreeUnitCode = $_POST['paramUnitCode'];
...
function copyTablePromItemPromStepPromStepFreeItem($conn, $paramPromNo, $paramPromType, $paramOldPromNo, $succss, $last_modified) {
    ...
    $sqlPromStep = "INSERT INTO PromStep "
            ...
            . "BreakUnitCode,BreakUnitFactor,BreakAmt,DiscPer,DiscBaht,"
            . "PromStepNote,last_modified)"
            . "SELECT PromType,'$paramPromNo',PromCode,Step,PromStepType,"
            . "MinimumSKU,BreakQty,BreakUnitCode,BreakUnitFactor,BreakAmt,"
            . "DiscPer,DiscBaht,PromStepNote,'$last_modified' "
            . "from PromStep "
    ...
    if (!$rsPromStep) {
        ...
        $sqlPromStepFreeItem = "INSERT INTO PromBundleFreeItem"
                . "(PromType,PromNo,Step,Seq,FreeItemCode,FreeQty,FreeUnitCode,"
                . "FreeUnitFactor,GLAccount,last_modified) "
                . "select PromType,'$paramPromNo',Step,Seq,FreeItemCode,FreeQty,"
...
//#----- END: Copy promotion item -------------------------------------------#//


//#----- START: Update Foriegn Table start ----------------------------------#//
...
if ($paramAction == "add") {
    ...
    if (checkAlreadyId($conn, $sqlCheck) == "Already-Id") {
        ...
        $sql = "INSERT INTO PromHeader"
                . "(PromType,PromNo,PromDesc,StartDate,EndDate,Priority,"
                . "BreakBy,DiscFor,DiscPer,DiscBaht,LimitFreeQty,LimitDiscBaht,"
                . "FreeUnitCode,last_modified) "
                . "VALUES('$paramPromType','$paramPromNo','$paramPromDesc','$paramStartDate','$paramEndDate','$paramPriority',"
                . "'$paramBreakBy','$paramDisCountFor','$paramDiscPer','$paramDiscBaht','$paramLimitFreeQty','$paramLimitDiscBath',"
                . "'$paramFreeUnitCode','$last_modified')";
...
if ($paramAction == "delete") {
    ...
    /* deleteForiegnTable */
    /*
      $sql="DELETE FROM PromHeader WHERE PromNo='$paramPromNo' and PromType='$paramPromType'";
...
//#----- START: Delete Foriegn Table start ----------------------------------#//
...
//#----- END: Delete Foriegn Table start ------------------------------------#//

//#----- START: delete table for reference ----------------------------------#//
function deleteTablePromItemPromStepPromStepFreeItem($conn, $paramPromNo, $paramPromType) {
    //delete PromStepFreeItem start
    $sqlPromStepFreeItem = "DELETE FROM PromBundleFreeItem "
...
//#----- END: delete table for reference ------------------------------------#//

//#----- START: deleteMainPromHeader start ----------------------------------#//
...
//#----- END: deleteMainPromHeader start ------------------------------------#//
...
if ($paramAction == "edit") {
    ...
    if (!$rs) {
        ...
        $sqlBranch = "SELECT * FROM PromOfBranch where PromNo ='$paramPromNo'";
        $rsBranch = odbc_exec($conn, $sqlBranch);
        ...
        $sqlShopType = "SELECT * FROM PromOfShopType where PromNo ='$paramPromNo'";
        $rsShopType = odbc_exec($conn, $sqlShopType);
        if ($rsShopType) {
            $j = 0;
        ...
        $sqlSalesTeam = "SELECT * FROM PromOfSalesTeam where PromNo ='$paramPromNo'";
        $rsSalesTeam = odbc_exec($conn, $sqlSalesTeam);
        if ($rsSalesTeam) {
            $k = 0;
        ...
        //FreeUnitFactor
        //LimitDiscBaht
        echo"[\"$PromNo\",\"$PromDesc\",\"$StartDate\",\"$EndDate\",\"$Alway\",\"$Priority\",[$BranchCode],[$ShopTypeCode],[$SalesTeam],\"$BreakBy\",\"$DiscFor\",\"$DiscPer\",\"$DiscBaht\",\"$LimitFreeQty\",\"$FreeUnitCode\",\"$FreeUnitFactor\",\"$LimitDiscBaht\"]";
    }
    odbc_close($conn);
}
//update data 
if ($paramAction == "editAction") {
    ...
    $sql = "UPDATE PromHeader SET PromDesc='$paramPromDesc',StartDate='$paramStartDate',EndDate='$paramEndDate',Priority='$paramPriority',last_modified='$last_modified',BreakBy='$paramBreakBy', DiscFor='$paramDisCountFor',DiscPer='$paramDiscPer',DiscBaht='$paramDiscBaht',LimitFreeQty='$paramLimitFreeQty',LimitDiscBaht='$paramLimitDiscBath',FreeUnitCode='$paramFreeUnitCode'
		WHERE PromNo='$paramPromNo' and PromType='$paramPromType'";
...
if ($paramAction == "showData") {
    ...
    if ($paramPromDesc != "" or ( $paramStartDate != "" and $paramEndDate != "")) {
        $sql = "SELECT * FROM PromHeader where (PromType='$paramPromType' and PromDesc like '%$paramPromDesc%') or (StartDate >= '$paramStartDate' and EndDate<='$paramEndDate' and  PromType='$paramPromType') order by PromNo";
    } else {
        $sql = "SELECT * FROM PromHeader where PromType='$paramPromType' order by PromNo";
    }
    ...
    $htmlShowData.= "<col style=\"width:14%\" />";
    $htmlShowData.= "<col style=\"width:14%\" />";
    $htmlShowData.= "<col style=\"width:38%\" />";
    $htmlShowData.= "</colgroup>";
    ...
    while (odbc_fetch_row($rs)) {
		...
		<td>		
                    ...
                    <button type=\"button\" class=\"btn btn-primary btn-xs btnPromStep\"  id=\"idPromStep-$PromNo\">Promotion Step </button>
                    <button type=\"button\" class=\"btn btn-primary btn-xs btnAddItem\" id=\"idAdd-$PromNo\" data-toggle=\"modal\" data-target=\".bs-addFreeItem-modal-lg\">Add Free Item </button>
...
//search data for discount by item start.
if ($paramAction == "searchPromNo") {
    $sql = "SELECT * FROM PromHeader where PromType='$paramPromType' and PromType='$paramPromType'";
    $rs = odbc_exec($conn, $sql);
    if (!$rs) {
        exit("Error in SQL");
    } else {
...
//search data for discount by item end.
?>