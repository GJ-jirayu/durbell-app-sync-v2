...
if ($paramAction == "edit") {
    ...
    $sql = "SELECT PromItem.*,PromGroup.GroupDesc "
            . "FROM PromItem "
...
if ($paramAction == "showData") {
    $sql = "SELECT PromItem.*,PromGroup.GroupDesc,unit.unitname "
            . "FROM PromItem "
            . "inner join PromGroup on PromItem.PromCode=PromGroup.GroupCode "
            . "left join unit on unit.unitcode = PromItem.FreeUnitCode "
            . "where PromType='$paramPromType' "
            . "and PromNo='$paramPromNo' "
            . "order by PromNo, PromCode";
    ...
    while (odbc_fetch_row($rs)) {
        ...
        $GroupCode = iconv("tis-620", "utf-8", odbc_result($rs, "PromCode"));
        $BreakBy = iconv("tis-620", "utf-8", odbc_result($rs, "BreakBy"));
        $Description = iconv("tis-620", "utf-8", odbc_result($rs, "GroupDesc"));
        $DiscFor = iconv("tis-620", "utf-8", odbc_result($rs, "DiscFor"));
        $LimitFreeQty = iconv("tis-620", "utf-8", odbc_result($rs, "LimitFreeQty"));
        $FreeUnitCode = iconv("tis-620", "utf-8", odbc_result($rs, "unitname"));
        $LimitDiscBaht = iconv("tis-620", "utf-8", odbc_result($rs, "LimitDiscBaht"));


        $htmlShowData.= "<tr><td>$PromNo</td>";
        $htmlShowData.= "<td>$GroupCode</td>";
        ...
        $htmlShowData.="
                        <td>
                            ...
                            <button type=\"button\" class=\"btn btn-primary btn-xs btnPromStep\"  id=\"idPromStep-$GroupCode\">Promotion Step </button>
