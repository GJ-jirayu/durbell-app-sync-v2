...
if ($paramAction == "add") {
    ...
    for ($i = 0; $i < $itemNo; $i++) {
        ...
        if (checkAlreadyId($conn, $sqlCheck) == "Already-Id") {
            ...
            $UnitFactorStr = "select UnitFactor from UnitOfItem where ItemCode = '$paramPromCode[$i]' and UnitCode = '$paramBreakUnitCode' ";
            $UnitFactorRs = odbc_exec($conn, $UnitFactorStr);
            $UnitFactor = iconv("tis-620", "utf-8", odbc_result($UnitFactorRs, "UnitFactor"));
            if(empty($UnitFactor)){
                $BreakUnitFactor = "1";
            } else {
                $BreakUnitFactor = $UnitFactor;
            }
		...
		VALUES('$paramPromType','$paramPromNo','$paramPromCode[$i]','$paramStep','$paramPromStepType','$paramMinimumSKU','$paramBreakQty','$paramBreakUnitCode','$BreakUnitFactor','$paramBreakAmt','$paramDiscPer','$paramDiscBaht','$paramPromStepNote','$last_modified')";
...
//    $countStep = "select * from PromStep where PromType='$paramPromType' and PromNo='$paramPromNo' and Step='$paramStep' ";
//    $countStepRs = odbc_exec($conn, $countStep);
//    $rowNum = odbc_num_rows($countStepRs);
//
//    if ($rowNum <= 1) { echo "11111";
...
left join Item on PromStep.PromCode=Item.ItemCode
left join PromGroup pg on PromStep.PromCode=pg.GroupCode
where PromType='$paramPromType' and PromNo='$paramPromNo'  order by Step";
